...
export function liveMovements<T extends MovementLike>(movements: T[]): T[] {
  const reversed = new Set(
    movements.filter((m) => m.reversalOfId).map((m) => m.reversalOfId!),
  );
...
export function tallyPopup(
  ...
  return {
    ...
    sold: sum(
      (m) =>
        m.fromLocationId === popupLocationId && m.reason === REASON_CODES.SALE,
    ),
    sample: sum(
      (m) =>
        m.fromLocationId === popupLocationId &&
        m.reason === REASON_CODES.SAMPLE,
    ),
...
export function popupDisplayStatus(
  ...
  return overdue && status === POPUP_STATUS.ACTIVE
    ? POPUP_STATUS.SETTLING
    : status;
}
...
export async function getPopupList() {
  const popups = await db.popup.findMany({
    include: {
      location: { include: { lots: { where: { quantity: { gt: 0 } } } } },
      movements: true,
    },
...
export async function getPopupDetail(popupId: number) {
  ...
  for (const line of popup.planLines) {
    touch(line.productId, line.product.name, line.product.unit).planned +=
      line.plannedQty;
  }
  for (const m of live) {
    const row = touch(m.productId, m.product.name, m.product.unit);
    if (m.toLocationId === popup.locationId) row.shipped += m.quantity;
    if (m.fromLocationId === popup.locationId && m.reason === REASON_CODES.SALE)
      row.sold += m.quantity;
    if (
      m.fromLocationId === popup.locationId &&
      m.reason === REASON_CODES.SAMPLE
    )
  ...
  for (const lot of popupLots) {
    touch(lot.productId, lot.product.name, lot.product.unit).onHand +=
      lot.quantity;
  }
  ...
  return {
    ...
    byProduct: [...byProduct.values()].sort(
      (a, b) => b.shipped - a.shipped || a.name.localeCompare(b.name, "ko"),
    ),
...
export function popupReport(
  ...
  const rate = (shipped: number, consumed: number) =>
    shipped > 0 ? consumed / shipped : 0;
  const rows = byProduct
    .filter((p) => p.shipped > 0)
    .map((p) => ({
      ...p,
      consumed: p.sold + p.sample,
      rate: rate(p.shipped, p.sold + p.sample),
    }))
...
export async function settlePopupTx(
  ...
  if (lots.length === 0)
    throw new Error("팝업에 남은 재고가 없습니다. 반출 기록을 확인하세요");
  ...
  for (const lot of lots) {
    ...
    if (!Number.isInteger(returned) || returned < 0)
      throw new Error("잔여 수량이 올바르지 않습니다");
    if (returned > lot.quantity)
      throw new Error(
        `반출한 수량보다 많이 돌아올 수 없습니다 (반출 ${lot.quantity}, 입력 ${returned})`,
      );
  ...
  for (const [productId, sample] of sampleOf) {
    if (sample < 0 || !Number.isInteger(sample))
      throw new Error("시식·증정 수량이 올바르지 않습니다");
  ...
  for (const lot of lots) {
    ...
    if (wantSample > 0) {
      restSample.set(
        lot.productId,
        (restSample.get(lot.productId) ?? 0) - wantSample,
      );
...
export function popupPeriod(start: Date, end: Date) {
  const f = (d: Date) =>
    dateOnly(d).toISOString().slice(0, 10).replace(/-/g, ".");
