  ...
  return (
    <main className="pb-16">
      ...
      <p className="border-b border-line bg-dim px-4 py-2.5 text-[11.5px] leading-relaxed text-[#5b5570]">
        팝업은 <b>여러 번 반출했다가 마지막에 한 번 정산하는 임시 거점</b>
        입니다. 현장에서 판매를 건별로 찍지 않고, 돌아온 실물 수량으로 판매량을
        역산합니다.
      </p>

      {running.length === 0 && (
        <p className="px-4 py-12 text-center text-[13px] text-sub">
          진행 중인 팝업이 없습니다
        </p>
      )}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-[3px] text-[11px] text-sub tnum">
                  {popupPeriod(p.startDate, p.endDate)} · 누적 반출 {p.shipped}
                  개 · 현재 보유 <b className="text-acc">{p.onHand}개</b>
                </p>
              </div>
      ...
      {closed.length > 0 && (
        <>
          ...
          {closed.map((p) => {
            ...
            const rate =
              p.shipped > 0 ? Math.round((consumed / p.shipped) * 100) : 0;
            return (
              <Link
                key={p.id}
                href={`/popups/${p.id}`}
              ...
              >
                <div>
                  ...
                  <p className="mt-[3px] text-[11px] text-sub tnum">
                    반출 {p.shipped} · 판매 {p.sold} · 시식 {p.sample} · 반입{" "}
                    {p.returned}
                  </p>
                </div>
                <span className="text-[12.5px] font-extrabold text-acc tnum">
                  소진율 {rate}%
                </span>
              </Link>
            );
